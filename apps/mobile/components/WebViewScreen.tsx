...
export default function WebViewScreen({ path, showHeader = false }: WebViewScreenProps) {
  ...
  const handleShouldStartLoadWithRequest = useCallback((request: WebViewNavigation) => {
    ...
    return true; // 일반 URL은 WebView에서 로딩
  }, []);
  ...
  const handleMessage = useCallback((event: WebViewMessageEvent) => {
    try {
      ...
      switch (message.type) {
        case 'OAUTH_LOGIN':
          // 웹에서 OAuth 요청이 오면 시스템 브라우저에서 열기
          const { url: oauthUrl } = message.payload;
          if (oauthUrl) {
