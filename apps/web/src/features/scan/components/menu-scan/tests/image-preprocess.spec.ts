...
test.describe('이미지 전처리 훅(useImagePreprocess) 기능 테스트', () => {
   ...
   */
  test('crop()을 호출하면 이미지가 지정된 영역으로 잘린다', async ({
    page,
  }) => {
    // 테스트 이미지 파일 업로드
    const fileInput = page.locator('[data-testid="image-input"]');
  ...
  test('전처리 후에도 원본 이미지가 보존된다', async ({ page }) => {
    ...
    // 원본 이미지 유지 확인
    const originalPreserved = await page
      .locator('[data-testid="original-preserved"]')
      .textContent();
   ...
   */
  test('autoCrop()을 호출하면 자동으로 메뉴판 영역이 감지된다', async ({
    page,
  }) => {
    // 테스트 이미지 파일 업로드
    const fileInput = page.locator('[data-testid="image-input"]');
   ...
   */
  test('optimizeForOCR()를 호출하면 OCR 인식률 향상을 위해 최적화된다', async ({
    page,
  }) => {
    // 테스트 이미지 파일 업로드
    const fileInput = page.locator('[data-testid="image-input"]');
