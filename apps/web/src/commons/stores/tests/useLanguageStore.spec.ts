...
test.describe('useLanguageStore Hydration 테스트', () => {
  ...
  test.describe('성공 시나리오', () => {
    ...
    test('hydration 완료 후 hydrated 상태가 true여야 한다', async ({
      page,
    }) => {
    ...
    test('언어 변경(setLanguage) 시 localStorage가 업데이트되어야 한다', async ({
      page,
    }) => {
      // 페이지 로드
      await page.goto(TEST_PAGE);
    ...
    test('useTranslation 훅 사용 시 hydration 완료 후 올바른 번역이 표시되어야 한다', async ({
      page,
    }) => {
      // localStorage에 영어 저장
      await page.evaluate((key) => {
  ...
  test.describe('실패/엣지 시나리오', () => {
    test('localStorage가 비어있을 때 기본 언어(ko)로 시작해야 한다', async ({
      page,
    }) => {
      // localStorage 비우기
      await page.evaluate((key) => {
    ...
    test('localStorage에 잘못된 포맷이 저장된 경우 기본 언어(ko)로 fallback해야 한다', async ({
      page,
    }) => {
      // 잘못된 포맷으로 저장
      await page.evaluate((key) => {
    ...
    test('localStorage에 유효하지 않은 언어가 저장된 경우 기본 언어(ko)로 fallback해야 한다', async ({
      page,
    }) => {
      // 유효하지 않은 언어로 저장
      await page.evaluate((key) => {
...
test.describe('코드상 초기값 검증', () => {
  test('useLanguageStore 소스 코드에서 hydrated 초기값이 false인지 확인', async ({
    page,
  }) => {
